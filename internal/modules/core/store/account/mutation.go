...
func CreateUser(ctx context.Context, user *models.User) error {
	logrus.WithField("email", user.Email).Info("Creating new user")
...
func CreateSession(ctx context.Context, session *models.UserSession) error {
	logrus.WithField("user_id", session.UserID).Info("Creating session for ", session.UserID)
...
func DeleteUserSession(ctx context.Context, userID snowflake.ID, refreshToken string) error {
	logrus.WithField("user_id", userID).Info("Deleting session for ", userID)
...
func DeleteUserAllSessions(ctx context.Context, userID snowflake.ID) error {
	logrus.WithField("user_id", userID).Info("Deleting all sessions for ", userID)
