...
func CreateDirectMessage(ctx context.Context, msg *models.DirectMessage) *appError.Error {
	...
	if msg.ID == 0 {
		...
		return appError.NewBadRequest("message ID is required")
	}
	if msg.Content == "" && msg.FileURL == nil {
		logrus.Error("message must have content or file to create message")
		return appError.NewBadRequest("message must have content or file")
	}
	if msg.UserID == 0 {
	...
	if msg.ConversationID == 0 {
		...
		return appError.NewBadRequest("conversation ID is required")
	...
	if err != nil {
		...
		return appError.NewInternal("Failed to insert the message in direct messages")
	}
